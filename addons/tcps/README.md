...
- **Tricks and the score** (`CTrickComponent`, `CSkaterScoreComponent`, `SkateTricks`): a flip (Square in THUG) or a grab (Circle) in the air is named by the direction held, from tables at the top of `skate_tricks.gd` (`Kickflip`, `Heelflip`, `Pop Shove-It`, `Impossible`, `Hardflip`; `Melon`, `Indy`, `Stalefish`, `Nosegrab`, `Tailgrab`); a grind is named the same way when the rail is taken (`50-50`, `Boardslide`, `Lipslide`, `Nosegrind`, `5-0`), a manual is a `Manual` or a `Nose Manual`. A flip takes 0.45 s and landing before it is done is a bail; a grab lasts while the button is held and scores the hold, as grinds and manuals do. The yaw turned in the air is counted on landing to the nearest half turn with THUG's `spin_count_slop` of 60 degrees (`FS 180`, `BS 360`), and a landing square across the roll is a bail. Every trick since the last clean landing is the combo, worth its points times its count; a clean landing banks it after a quarter second's grace, in which a manual, a grind or a revert (`focus` or `shoot` within 0.3 s of a vert landing, THUG's L2 and R2) carries it on. A bail loses it.
...
Not yet ported: special tricks and the special meter, skitching, and the walking skater. Trick points for the lip tricks, wallride, wallplant, transfers and acid drop are THPS-era values, since THUG keeps its trick scores in scripts that are not in the repository.

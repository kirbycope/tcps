...
- **Camera** (`CSkaterCam::Update`): runs in the physics tick after the board has moved the skater, from a node that is not dragged by the skater's body, reads a smoothed floor normal, and rides through the one-tick contact flicker steep transitions produce.

Not yet ported: lip tricks, wall rides and wall plants, spine transfers and acid drops, flip and grab tricks, spin counting, reverts and the score, and THUG's camera zooms and vert cam.

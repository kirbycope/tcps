...
func test_a_kick_is_a_steady_acceleration_to_a_cap_and_coasting_keeps_the_speed() -> void:
	...
	assert_between(after_half_a_second, 2.0, 4.0, "Half a second of kicking is about three metres a second, not top speed")
	await wait_physics_frames(180)
	assert_almost_eq(_speed(), Skateboard.KICK_MAX_SPEED, 0.6, "Three and a half seconds in, the kick has reached its cap")
	Input.action_release(&"move_up")
	var before: float = _speed()
	await wait_physics_frames(120)
	assert_gt(_speed(), before * 0.9, "Two seconds of coasting loses less than a tenth: there is no rolling friction, only the wind")
...
func test_a_turn_keeps_the_speed_and_puts_it_all_along_the_facing() -> void:
	...
	assert_almost_eq(_speed(), 10.0, 0.4, "Three quarters of a second of turning at speed keeps the speed")
